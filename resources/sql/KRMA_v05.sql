...
/*  Created On : 04-Okt-2017 19:37:46 				*/
/*  DBMS       : MySql 						*/
/* ---------------------------------------------------- */
...
(
	`ID` int not null IDENTITY,
	`REPORT_ID` VARCHAR(10) NOT NULL COMMENT 'Eindeutige GUID fuer eine Report Speicherung',
	`REPORT_DATE` DATETIME NOT NULL COMMENT 'Datum und Zeitpunkt der Jenkins Abfrage.',
	`REPORT_ENV_SHORT` VARCHAR(8) 	 NULL COMMENT 'Falls Umgebung f�r diesen Report eindeutig, kann dieses Feld bef�llt werden.',
	`JENKINS_PROJECT` VARCHAR(80) 	 NULL COMMENT 'Name des Jenkins Project. Idealerweise identisch mit dem Komponenten-Namen in /km/dependencies.',
	`JENKINS_TECHNOLOGY` VARCHAR(30) 	 NULL,
	`JENKINS_JOB` VARCHAR(120) 	 NULL COMMENT 'Name des Jenkins-Job der ausgefuert wurde.',
	`JENKINS_BUILD_ID` VARCHAR(30) 	 NULL COMMENT 'Build ID des Jenkins-Job. Sollte zudem der Namens-Suffix des Artifactory Paketes sein.',
	`JENKINS_BUILD_DATE` DATETIME 	 NULL COMMENT 'Datum und Zeitpunkt des Jenkins Build',
	CONSTRAINT `PK_PACKAGE_COCKPIT_STAGING` PRIMARY KEY (`ID` ASC)
)
