...
(
	`ID` int not null IDENTITY,
	`COMP_ID` DECIMAL(10,0) 	 NULL COMMENT 'Referenz fuer diesen Hyperlink in einer Componente',
	`PACK_ID` DECIMAL(10,0) 	 NULL COMMENT 'Referenz fuer diesen Hyperlink in einem Package',
	`REL_ID` DECIMAL(10,0) 	 NULL COMMENT 'Referenz fuer diesen Hyperlink in einem Release',
	`ENV_ID` DECIMAL(10,0) 	 NULL COMMENT 'Referenz fuer diesen Hyperlink in einem Environment',			
...
ALTER TABLE `HYPERLINK` 
 ADD CONSTRAINT `FK_HYPERLINK_COMPONENT`
	FOREIGN KEY (`COMP_ID`) REFERENCES `COMPONENT` (`ID`) ON DELETE No Action ON UPDATE No Action
;

ALTER TABLE `HYPERLINK` 
 ADD CONSTRAINT `FK_HYPERLINK_ENVIRONMENT`
	FOREIGN KEY (`ENV_ID`) REFERENCES `ENVIRONMENT` (`ID`) ON DELETE No Action ON UPDATE No Action
;

ALTER TABLE `HYPERLINK` 
 ADD CONSTRAINT `FK_HYPERLINK_PACKAGE`
	FOREIGN KEY (`PACK_ID`) REFERENCES `PACKAGE` (`ID`) ON DELETE No Action ON UPDATE No Action
;

ALTER TABLE `HYPERLINK` 
 ADD CONSTRAINT `FK_HYPERLINK_RELEASE`
	FOREIGN KEY (`REL_ID`) REFERENCES `RELEASE` (`ID`) ON DELETE No Action ON UPDATE No Action
;
